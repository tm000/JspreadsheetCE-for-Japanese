	...
	jce.plugin = (optVal = {}) => {
		...
		return function() {
			...
			plugin.init = function(worksheet) {
				...
				worksheet.content.addEventListener("mousemove", onResizing);
				worksheet.content.addEventListener("touchmove", onResizing);
				// エディタをspreadsheetの一部として追加
				worksheet.content.appendChild(editor);
				// カスタムエディタを中身のあるものに入れ替える
				worksheet.options.columns.forEach(c => {
					if (c.type == jce.editor) {
						c.type = customeditor;
					}
				})
				// resetSelectionメソッドにオプション(hideSelection)のロジックを追加
				const defaultResetSelect = worksheet.resetSelection;
				worksheet.resetSelection = function() {
					defaultResetSelect();
