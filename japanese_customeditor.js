var japaneseCustomEditor = (() => {
	var jce = {};
	var setEvent = false;
	var activespread, activeHideSelection;
	...
	jce.setup = function(spreadsheet, optVal = {}) {
		...
		var options = {...defaultOptions, ...optVal};
		...
		function updateEditorPosition() {
			...
			x = parseInt(x);
			y = parseInt(y);
			if (spreadsheet.colgroup.length - 1 < x || spreadsheet.rows.length - 1 < y) {
				// redoにより選択していたセルがなくなった場合、有効なセルを再選択する
				if (spreadsheet.colgroup.length - 1 < x) {
					x = spreadsheet.colgroup.length - 1;
					editor.setAttribute('data-x', x);
				}
				if (spreadsheet.rows.length - 1 < y) {
					y = spreadsheet.rows.length - 1;
		...
		function updateEditorSize(x, y) {
			let cornerCell = spreadsheet.headerContainer.children[0].getBoundingClientRect();
			let contentRect = spreadsheet.content.getBoundingClientRect();
			let cell = spreadsheet.getCellFromCoords(x, y)
			let info = cell.getBoundingClientRect();
			editor.style.minWidth = (info.width) + 'px';
			editor.style.minHeight = (info.height) + 'px';
			const scrollTop = spreadsheet.content.scrollTop;
			const scrollLeft = spreadsheet.content.scrollLeft;
		...
		var defaultParseCSV = spreadsheet.parseCSV;
		spreadsheet.parseCSV = function(str, delimiter) {
		...
		var defaultSelect = spreadsheet.textarea.select;
		spreadsheet.textarea.select = function() {
		...
		var defaultCopy = spreadsheet.copy;
		spreadsheet.copy = function(highlighted, delimiter, returnData, includeHeaders, download) {
		...
		var defaultUpdateCornerPosition = spreadsheet.updateCornerPosition;
		spreadsheet.updateCornerPosition = function() {
			defaultUpdateCornerPosition();
			// Resizing is ongoing
			if (spreadsheet && spreadsheet.resizing) {
				let x = editor.getAttribute('data-x');
				let y = editor.getAttribute('data-y');
				if (x && y) {
					updateEditorSize(x, y);
		...
		var defaultResetSelection = spreadsheet.resetSelection;
		spreadsheet.resetSelection = function(blur) {
			if (blur && !options.hideSelection) return;
			defaultResetSelection(blur);
			editor.style.display = 'none';
		}

		spreadsheet.options['onselection'] = function(e,x,y,x2,y2) {
			// 異なるjspreadsheet間を移動した場合にもhideSelectionを適用する
			const newSpread = e.jexcel;
			if (activespread != newSpread) {
				if (activespread && !activeHideSelection && activespread.selectedContainer) {
					const selected = activespread.selectedContainer;
		...
		// カスタムエディタを中身のあるものに入れ替える
		spreadsheet.options.columns.forEach(c => {
			if (c.editor == jce.editor) {
				c.editor = customeditor;
			}
		})

		// エディタをspreadsheetの一部として追加
		spreadsheet.content.appendChild(editor);
	}
	return jce;
})();
